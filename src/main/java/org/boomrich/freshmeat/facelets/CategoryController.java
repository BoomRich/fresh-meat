...
import org.boomrich.freshmeat.sessions.CategoryFacade;
...
public class CategoryController implements Serializable {
    ...
    @EJB
    private org.boomrich.freshmeat.sessions.CategoryFacade ejbFacade;
    ...
    public List<String> getFirstCategorys() {
        Set<String> firstCategory = new TreeSet<String>();
        for (Category c : getFacade().findAll()) {
            firstCategory.add(c.getCategoryName().split("@", 2)[1]);
        }
    ...
    public List<String> getSecondCategorys(String firstCategory) {
        Set<String> secondCategory = new TreeSet<String>();
        for (Category c : getFacade().findAll()) {
            if (c.getCategoryName().endsWith(firstCategory)) {
                secondCategory.add(c.getCategoryName().split("@", 2)[0]);
            }
        }
        ArrayList<String> sStrings = new ArrayList<>();
        for (String string : secondCategory) {
            sStrings.add(string);
        }
        return sStrings;
    }
